...
if($stmt->rowCount() > 0)
{
	...
	if($dh = opendir($test_dir))
	{
		while (($file = readdir($dh)) !== false){
			...
			{
				?>
                <div class="audio-li select-audio"><a 
					href="<?php echo $filename;?>" 
					title="<?php echo basename($filename);?>" 
					data-name="<?php echo basename($filename);?>"><span class="audio-80"><span><?php echo basename($filename);?></span></span></a></div>
                <?php
			}
		}
		closedir($dh);
