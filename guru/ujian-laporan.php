...
$(document).ready(function(e) {
    $(".basic-competence-score table").each(function() {
        var $this = $(this);
        var newrows = [];
...
<form id="searchform" name="form1" method="get" action="">
    ...
    <?php 
    $sql2 = "select * from `edu_class` where `active` = '1' and `school_id` = '$school_id' and `teacher_id` = '$auth_teacher_id' order by `order` asc ";
...
<div class="search-pagination-label"><?php echo $pagination->start;?>-<?php echo $pagination->end;?>/<?php echo $pagination->total_record;?></div>
  ...
  <thead>
    ...
    <tr class="row-data<?php if($data['lewat']) echo ' data-error';?>">
      ...
      <td align="right"><?php echo $picoEdu->numberFormatTrans($data['percent']);?></td>
      <td align="right"><?php echo $picoEdu->numberFormatTrans($threshold);?></td>
      <td><?php if($data['percent'] >= $threshold) echo 'Ya'; else echo 'Tidak';?></td>
...
<form name="rowform" method="post" action="">
  ...
  <thead>
    <tr>
        ...
        <td>Kelas</td>
        <td>Terakhir</td>
        <td width="50" align="right">Koleksi</td>
