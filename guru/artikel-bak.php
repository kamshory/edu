...
$arrc = array();
while(($datac = mysql_fetch_assoc($resc)))
{
	$arrc[] = $datac;
}
if(@$_GET['option'] == 'add')
...
else
{
	if(isset($_GET['article_id']))
	{
		...
		where (`edu_article`.`active` = '1' or `edu_article`.`member_create` = '$teacher_id') $sql_filter_article ";
		include_once dirname(__FILE__)."/lib.inc/header.php";
		$stmt = $database->executeQuery($sql);
		if($stmt->rowCount() > 0)
		{
...
$sql_filter .= " and (`edu_article`.`teacher_create` = '$teacher_id' )";
...
where (`edu_article`.`active` = '1' or `edu_article`.`member_create` = '$admin_id') $sql_filter 
