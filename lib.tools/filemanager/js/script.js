...
function contextMenuListFile(filetype, filepath, fileurl, attr) {
	...
	if (filetype.indexOf('image') == 0) {
		...
		cm = [
			...
			{ 'caption': 'Compress Audio', 'linkurl': 'javascript:compressAudio(\'' + filepath + '\')', 'classname': 'play' },
			{ 'caption': 'Compress File', 'linkurl': 'javascript:compressFile(\'' + filepath + '\')', 'classname': 'compress' },
			{ 'caption': 'Set Permission', 'linkurl': 'javascript:changePermission(\'' + filepath + '\')', 'classname': 'permission' },
			{ 'caption': 'Download File', 'linkurl': '../../../' + fileurl + '" target="_blank', 'classname': 'download' },
			{ 'caption': 'Force Download File', 'linkurl': 'javascript:forceDownloadFile(\'' + filepath + '\')', 'classname': 'download' },
...
function previewFile(url, width, height, fullsize, frommenu) //NOSONAR
{
	...
	let previewURL = '../../' + url;
	if (fullsize) {
		html = '<img src="' + previewURL + '" width="' + w + '" height="' + h + '" class="image2zoomout" title="' + bn + '" onclick="previewFile(\'' + url + '\', \'' + width + '\', \'' + height + '\', false);" />';
	}
	else {
		if (width > 500) {
			w = 500;
		...
		html = '<img src="' + previewURL + '" width="' + w + '" height="' + h + '" class="image2zoomin" title="' + bn + '" onclick="previewFile(\'' + url + '\', \'' + width + '\', \'' + height + '\', true);" />';
	}
	let prevThumb = {};
...
function playVideo(url, type) {
	...
	let width = 500;
	let height = 375;
	if (videoDimensions[previewURL] != null) {
		height = width * videoDimensions[previewURL].height / videoDimensions[previewURL].width;
	}
	let dheight = $(document).height();
	let dwidth = $(document).width();
	if (height > (dheight - 60)) {
		height = dheight - 60;
		width = height * videoDimensions[previewURL].width / videoDimensions[previewURL].height;
	}
	if (width > (dwidth - 60)) {
		width = dwidth - 60;
		height = width * videoDimensions[previewURL].height / videoDimensions[previewURL].width;
	...
	if (type == 'iframe') {
		html = '<iframe frameborder="0" hspace="0" vspace="0" marginheight="0" marginwidth="0" scrolling="auto" src="' + url + '" width="' + width + '" height="' + height + '" /></ifame>';
	}
	else if (type == 'html5') {
		html = '<video src="' + previewURL + '" poster="style/images/movie.png" controls="true" width="' + width + '" height="' + height + '"><object width="' + width + '" height="' + height + '" type="application/x-shockwave-flash" data="moxieplayer.swf"><param name="url" value="' + previewURL + '"><param name="src" value="moxieplayer.swf"><param name="allowfullscreen" value="true"><param name="allowscriptaccess" value="true"><param name="autoplay" value="true"><param name="flashvars" value="url=' + url + '&amp;poster=style/images/movie.png&amp;autoplay=true"></object></video>';
	}
	else if (type == 'moxie') {
		html = '<object width="' + width + '" height="' + height + '" type="application/x-shockwave-flash" data="moxieplayer.swf"><param name="url" value="' + previewURL + '"><param name="src" value="moxieplayer.swf"><param name="allowfullscreen" value="true"><param name="allowscriptaccess" value="true"><param name="autoplay" value="true"><param name="flashvars" value="url=' + previewURL + '&amp;poster=style/images/movie.png&amp;autoplay=true"></object>';
	}
	else
		html = '<object width="' + width + '" height="' + height + '" data="' + previewURL + '" type="application/x-mplayer2"><param name="url" value="' + previewURL + '" /></object>';
	{
...
function playAudio(url, type) {
	...
	if (type == 'iframe')
		...
		html = '<video src="' + previewURL + '" controls="true" width="320" height="50"><object width="320" height="50" data="' + url + '" type="application/x-mplayer2"><param name="url" value="' + previewURL + '" /></object></video>';
	else if (type == 'moxie')
		html = '<object width="320" height="50" type="application/x-shockwave-flash" data="moxieplayer.swf"><param name="url" value="' + previewURL + '"><param name="src" value="moxieplayer.swf"><param name="allowfullscreen" value="true"><param name="allowscriptaccess" value="true"><param name="autoplay" value="true"><param name="flashvars" value="url=' + previewURL + '&amp;poster=style/images/audio.png&amp;autoplay=true"></object>';
	else
		html = '<object width="320" height="50" data="' + previewURL + '" type="application/x-mplayer2"><param name="url" value="' + previewURL + '" /></object>';
	overlayDialog(html, 320, 50);
}
function compressAudio(path) {
	$('#common-dialog-inner').html('Compress audio will reduce the quality and file size. This process can not be undone.<br />Are you sure yo want to compress this audio?');
	$('#common-dialog').dialog({
		...
		{
			'Yes': function () {
				$('#common-dialog').parent().find('.ui-dialog-buttonpane').find('button').each(function (index, element) {
					$(this).prop('disabled', true);
				});
				$('#common-dialog').html('Please wait while audio file is being compressed.');
				...
				$.post('tool-file-operation.php' + q, { path: path }, function (answer) {
					if (answer == 'SUCCESS') {
						...
						alert('File type can not be compressed.');
					}
...
function previewPDF(url) {
	let previewURL = '../../' + url;
	let html = '<embed src="' + previewURL + '#toolbar=0&amp;navpanes=0&amp;scrollbar=0" width="720" height="400">';
	overlayDialog(html, 720, 400);
}
function previewSWF(url, width, height) {
	let previewURL = '../../' + url;
	let html = '<object width="' + width + '" height="' + height + '" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0"><param name="src" value="' + previewURL + '"><embed src="' + previewURL + '" width="' + width + '" height="' + height + '"></embed></object>';
	overlayDialog(html, width, height);
}
...
function renameFile(filepath, isdir) //NOSONAR
{
...
function uploadFile() {
	...
	$('#targetdir').val(dl);
	$.ajax({ type: "GET", url: "js/upload.min.js", dataType: "script" });
}
...
function changePermission(filepath) //NOSONAR
{
	let data = [];
	if (!filepath) {
...
function showPermission(perms) {
	...
	let p_user = parseInt(perms.substr(0, 1));
	let p_group = parseInt(perms.substr(1, 1));
	let p_world = parseInt(perms.substr(2, 1));
	let ur = (p_user >> 2) % 2;
	let uw = (p_user >> 1) % 2;
	let ue = (p_user) % 2;
	let gr = (p_group >> 2) % 2;
...
function initHotKey() {
	toggleFocusInput();
	$(document).on('keypress', function (e) //NOSONAR
	{
...
function FileSelectHandler(e) {
	...
	html += '<div class="file-upload" style="height:180px; overflow:auto;">';
	for (let i in files) {
		formData.append('images[]', files[i]);
		html += '<div class="file-upload-item" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">' + files[i].name + '</div>';
	}
	html += '</div>';
