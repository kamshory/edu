...
<label><input type="radio" name="compressimageonupload" class="compressimageonupload" value="1"<?php
		if ($compressimageonupload == 1) {
			echo ' checked="checked"';
		}?> /> Yes</label>
<label><input type="radio" name="compressimageonupload" class="compressimageonupload" value="0"<?php if ($compressimageonupload == 0) {
	echo ' checked="checked"';
		}?> /> No</label>
</td>
</tr>
...
<label><input type="radio" name="imageinterlace" class="imageinterlace" value="1"<?php 
if ($imageinterlace == 1) {
	echo ' checked="checked"';
}?> /> Yes</label>
<label><input type="radio" name="imageinterlace" class="imageinterlace" value="0"<?php 
if($imageinterlace==0) {
	echo ' checked="checked"';
}?> /> No</label>
</td>
...
<td><select name="imageformat" id="imageformat">
    ...
    ?>
    <option value="0"<?php if ($imageformat == 0) {
	echo ' selected';
	}?>>JPEG only</option>
	<option value="1"<?php if ($imageformat == 1) {
	echo ' selected';
	}?>>All formats</option>
  </select></td>
</tr>
...
$imageformat = getfmprofile('imageformat',$authblogid,0);
?>
<label><input type="checkbox" id="compress-image-cb" name="compress-image-cb" value="1"<?php if ($compressimageonupload == 1) {
		echo ' checked="checked"';
		} ?> onChange="setActiveCompress(this.checked)" /> Compress <?php 
		if ($imageformat == 0) {
		echo 'JPEG ';
		}
		?>Image Maximum <?php echo getfmprofile('maximagewidth',$authblogid,600);?>x<?php echo getfmprofile('maximageheight',$authblogid,800);?></label>
<?php
}
?>